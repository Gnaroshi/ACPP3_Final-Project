...
4. 선택 완료 후 결과 영역은 오늘 할 작은 미션을 먼저 전체폭으로 보여주고, 그 아래에 `추천 공식 자료 Top`과 지도 preview를 desktop 2열로 표시합니다.
5. Top 자료 아래에는 `추가 추천 자료`를 같은 결과 컨테이너 안에 이어서 보여주며, 각 자료 카드 오른쪽에 장소 확인 블록을 붙입니다.
6. `세부 조건 더 조정하기`는 동네, 검색어, 최대 부담도, 확인 방식처럼 결과를 더 좁히고 싶을 때만 엽니다.
...
- Desktop 1280px 이상에서 선택 완료 후 결과 영역은 `미션 전체폭 -> 추천 공식 자료 Top/지도 2열 -> 추가 추천 자료` 순서로 보여야 합니다.
...
- 네 조건 선택 후 mission/resource/map/action과 추가 추천 자료가 같은 결과 영역에서 보여야 합니다.
