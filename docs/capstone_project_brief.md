...
## 3. User Scenario
사용자는 먼저 hero와 프로젝트 설명에서 RebootRoute가 진단/상담 서비스가 아니라 인천 공식 자료를 오늘 할 작은 행동으로 정리하는 도구임을 확인합니다. 그 다음 조건 선택 전에 볼 공식 자료 preview에서 청년정책, 청년공간, 문화행사, 프로그램을 훑어본 뒤 오늘 쓸 수 있는 시간, 사람 만나는 부담, 먼저 보고 싶은 자료, 오늘 쓸 비용을 선택합니다. 네 조건이 모두 선택되면 화면은 오늘 할 작은 미션, 공식 출처 기반 자원 후보, 지도 위치, 기록 버튼을 보여줍니다. 사용자는 미션을 시작/완료/too-hard로 기록하거나, 프로그램 참여·지원 신청·지원 결과·미니 프로젝트 제출 outcome을 남길 수 있습니다.
...
1. `make pipeline`
2. `make dashboard`
3. hero, 프로젝트 설명, 공식 자료 preview 확인
4. 내 루트 탭에서 네 가지 조건 선택
5. 오늘 루트, 오늘 할 미션, 공식 자원, 지도 확인
