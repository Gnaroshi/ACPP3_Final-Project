...
사용자 데이터를 실제로 확보할 수 없으므로, 현재 대시보드의 입력은 데모 세션 상태입니다. 최신 대시보드는 사용자가 먼저 서비스 목적과 공식 자료를 확인한 뒤 조건을 고르는 구조입니다.

1. 상단 hero와 서비스 설명에서 RebootRoute의 범위 확인
2. 조건 선택 전에 볼 인천 공식 자료 preview 확인
3. 오늘 쓸 수 있는 시간, 사람 만나는 부담, 먼저 보고 싶은 자료, 오늘 쓸 비용 선택
4. 네 조건이 모두 선택되면 오늘 할 작은 미션, 가장 맞는 공식 자료, 지도 안내 확인
5. `오늘 이걸로 시작`, `완료`, `나중에`, `어려움`으로 progress/feedback 기록

Synthetic profile과 synthetic label은 모델 학습, API 호환성, 검증, MLOps 시연을 위해서만 남아 있습니다. 사용자 화면의 주 데이터는 `make official-data`로 수집한 공식 페이지 기반 resource입니다. 카드 설명, 부담도, 태그, 예상 소요시간은 공식 원문 그대로가 아니라 RebootRoute 추천 UX를 위한 파생 메타데이터이며, 데모 중 남기는 progress/feedback/outcome은 SQLite에 기록됩니다.
...
개발자/운영자 검증 화면은 기본 사용자 화면에 보이지 않습니다. 내부 ID, score, raw payload, 모델 보조 결과를 확인해야 할 때만 아래처럼 쿼리 파라미터를 붙여 엽니다.
...
- RebootRoute hero와 서비스 목적
- 프로젝트 설명: 공식 자료 확인, 작은 행동 추천, 지도와 기록 연결
- 조건 선택 전에 볼 공식 자료 preview carousel
- carousel의 `이전`/`다음` 버튼
- `전체 자료 보기`를 눌렀을 때만 열리는 공식 자료 목록

여기서 보이는 공식 자료는 추천 결과가 아니라 사용자가 조건을 고르기 전에 참고할 수 있는 수집 자료 preview입니다.

### 3.2 내 루트

실제 사용자에게 보여주는 메인 route builder입니다.

보이는 것:

- 오늘 쓸 수 있는 시간, 사람 만나는 부담, 먼저 보고 싶은 자료, 오늘 쓸 비용 선택
- `세부 조건 더 조정하기`를 눌렀을 때만 보이는 위치, 자료 범위, 비용, 부담도, 확인 방식, 에너지, 취업 부담, 불안, 상태 메모
- desktop 화면에서는 네 조건 카드가 한 줄로 정렬되고, mobile 화면에서는 한 장씩 읽기 쉽게 쌓임
- 선택 완료 후 가장 먼저 보이는 `오늘 바로 할 행동` 미션 카드
- 그 아래 `추천 공식 자료 Top` 카드와 지도 카드가 desktop 2열, mobile 1열로 정렬됨
- Top 자료 아래에 이어지는 `추가 추천 자료` 리스트. 각 자료는 이미지, 자료 설명, 오른쪽 장소 확인 블록을 한 카드 안에서 함께 보여줌
- 자원명, 자원 종류, 구/군, 공식 확인 기준
- 공식 출처명과 공식 페이지 링크
- 지도 위 추천 활동 장소와 길찾기 링크
- 미션 카드 아래의 `오늘 이걸로 시작`, `완료`, `나중에`, `어려움`
- `활동 결과 기록`을 눌렀을 때만 보이는 프로그램 참여, 지원 신청, 지원 결과, 미니 프로젝트 제출 기록 폼
...
사용자는 먼저 네 가지 선택지를 고릅니다. 네 조건이 모두 선택되기 전에는 추천 결과를 확정해서 보여주지 않습니다. 조건을 바꾸면 추천 미션, 추천 공식 자료 Top, 추가 추천 자료, 지도 위치가 즉시 다시 계산됩니다. 별도의 업데이트 버튼이나 재추천 버튼을 누르지 않습니다.

자원 후보는 구/군, 비용, 최대 부담도, 온라인 확인 가능 여부, 내 위치와의 거리 기준으로 정렬됩니다.

대시보드 CSS는 `src/rebootroute/dashboard/css/` 아래 역할별 파일로 분리되어 있습니다. 공통 토큰과 Streamlit 기본 위젯 보정은 `base.css`, hero와 프로젝트 설명은 `hero.css`, 공식 자료 carousel은 `resources.css`, 조건 선택은 `route.css`, 결과 카드와 지도 배치는 `cards.css`와 `results.css`, breakpoint는 `responsive.css`에서 관리합니다.
...
내부 검증용 화면입니다. 실제 사용자에게 보여주는 화면이 아닙니다. `?operator=1`에서만 기본 탭 아래에 표시됩니다.
...
여기에는 내부 ID와 score가 표시될 수 있습니다. 사용자 화면에서 숨긴 정보가 개발자/운영자 검증용으로만 남아 있는 구조입니다.
...
`sample_profiles.csv`, `sample_progress.csv`, `sample_outcomes.csv`는 synthetic MVP mock dataset입니다. 현재 기본 생성 규모는 profile 1000명, progress log 약 1900건, outcome event 약 1300건입니다. 모델 학습, API 테스트, 운영자 점검을 위한 더미 데이터이며 실제 사용자 상태, 실제 완료 이력, 실제 참여/지원 결과가 아닙니다.
...
1. synthetic profile/progress/outcome mock dataset 생성 및 기존 공식 resource CSV 보존
2. 데이터 검증
3. feature table 생성
4. stage model 학습
...
src/rebootroute/dashboard/components/*.py
src/rebootroute/dashboard/views/*.py
src/rebootroute/dashboard/css/*.css
src/rebootroute/dashboard/styles.py
```
...
개발자/운영자 검증 화면은 `?operator=1`로 접근했을 때만 표시됩니다. 기본 사용자 화면에는 내부 ID, score, point, raw payload가 보이지 않아야 합니다.
...
- hero → 프로젝트 설명 → 공식 자료 preview → 탭 → 내 루트 순서로 보이는지
- 네 조건 선택 전에는 추천 결과를 확정 노출하지 않는지
- 네 조건 선택 후 오늘 미션, 공식 자원, 지도, 기록 버튼이 보이는지
- 공식 출처 링크가 보이는지
