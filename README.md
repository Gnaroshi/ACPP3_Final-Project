...
1. 오늘 가능한 범위, 사람 만나는 정도, 찾고 싶은 것, 오늘 비용 선택
2. 현재 조건에 맞는 오늘 할 미션과 공식 자원 확인
3. 지도 preview에서 내 위치와 활동 장소 확인
4. `오늘 이걸로 시작`, `완료`, `나중에`, `어려움`으로 피드백 기록
...
- 오늘 가능한 범위, 사람 만나는 정도, 찾고 싶은 것, 오늘 비용 선택
- `더 조정하기`를 눌렀을 때만 보이는 위치, 자료 범위, 비용, 부담도, 확인 방식, 에너지, 취업 부담, 불안, 상태 메모
- 오늘 할 미션 카드
- 공식 출처 기반 인천 자원 카드와 이미지
- 자원명, 자원 종류, 구/군, 내 위치 기준 대략 거리
- 공식 출처명과 공식 페이지 링크
- 지도 위 내 위치와 활동 장소
- 하단 고정 action bar의 `오늘 이걸로 시작`, `완료`, `나중에`, `어려움`
...
사용자는 먼저 네 가지 선택지를 고릅니다. 조건을 바꾸면 추천 미션, 자원 후보, 지도 위치가 즉시 다시 계산됩니다. 별도의 업데이트 버튼이나 추천 루트 버튼을 누르지 않습니다.
